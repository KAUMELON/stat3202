...
$\mathbf{n}$ is the whole area size. Column 2 and 3 in the table 2.1 is the range of $\frac{\mathbf{x}_{\mathbf{1}}}{\mathbf{n}}$. once you get the computed $b$ and value of $\frac{\mathbf{x}_{\mathbf{1}}}{\mathbf{n}}$. You can calculate the corresponding adjusted $b$ using the table 2.1. using interpolation as follows.  
...
corresponding to *L*~1~ and *L*~2~ under the range of $\frac{\mathbf{x}_{\mathbf{1}}}{\mathbf{n}}$ in the table 2.1. Then using the formula  
...
Pair wise calculation of vertical values from the experiment observations shown in figure \@ref(fig:u1)
...
>Results show that the horizontal-strip MS is almost 3 times higher than
the vertical-strip MS, indicating that the trend of soil fertility was
more pronounced along the length than along width of the field.  
...
+---------+--------------+-------+--------+--------------+
| Sl. No. | Size of plot | Width | Length | No: of plots |
|         |              |       |        |              |
|         | (*x*)        |       |        | ($w$)        |
+=========+==============+=======+========+==============+
| 1       | 1            | 1     | 1      | 150          |
...
It is recommended that the reader should first read section
\@ref(smith) and then read this example. Here, I will illustrate how
variance per unit area for a plot of size 25m^2^ (denoted as *V*~25~) is
calculated. The width and length of the plot is 5m × 5m. The entire plot
area in figure \@ref(fig:u1) is divided in to six plots of size 25m^2^
as shown below.  
...
$V_{(x)} = \sum_{i = 1}^{w}\frac{{T_{i}}^{2}}{x} - \frac{\left( G \right)^{2}}{\text{rc}}$.
where $w = \frac{\text{rc}}{x}\ $is the total number of simulated plots
of size$\text{\ x}$ basic units. $r$ is the number of rows and $c$ is
the number of columns. $G$ is the total of all the basic units.  
...
$V_{(25)} = \frac{21366^{2} + 21433^{2}\ldots + 18811^{2}}{25} - \frac{\left( 122777 \right)^{2}}{150} = \ $`<!-- -->`{=html}218767.7133

Variance per unit area is given by $V_{x} = \frac{V_{(x)}}{rc - 1}$

$V_{25} = \frac{218767.7133}{149}\  = \ $`<!-- -->`{=html}1468.24
...
+---------+--------------+-------+--------+--------------+---------+-----+
| Sl. No. | Size of plot | Width | Length | No: of plots | *V~x~*  | C.V |
|         |              |       |        |              |         |     |
|         | (*x*)        |       |        | ($w$)        |         |     |
+=========+==============+=======+========+==============+=========+=====+
| 1       | 1            | 1     | 1      | 150          | 5577.88 | 9.1 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 2       | 2            | 2     | 1      | 75           | 4427.18 | 8.1 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 3       | 3            | 1     | 3      | 50           | 3733.36 | 7.5 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 4       | 5            | 1     | 5      | 30           | 2213.82 | 5.7 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 5       | 5            | 5     | 1      | 30           | 3066.42 | 6.8 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 6       | 6            | 2     | 3      | 25           | 3316.61 | 7   |
+---------+--------------+-------+--------+--------------+---------+-----+
| 7       | 10           | 2     | 5      | 15           | 2003.03 | 5.5 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 8       | 15           | 5     | 3      | 10           | 2268.41 | 5.8 |
+---------+--------------+-------+--------+--------------+---------+-----+
| 9       | 25           | 5     | 5      | 6            | 1468.24 | 4.7 |
+---------+--------------+-------+--------+--------------+---------+-----+
...
+--------------+---------+-------------+--------------+--------------+
| Size of plot | *V*~x~  | log(*V*~x~) | *Y* =        | *X* =        |
|              |         |             | log(*V*~x~)  | log(*x*)     |
| (*x*)        |         |             | -log(*V*~1~) |              |
+==============+=========+=============+==============+==============+
| 1            | 5577.88 | 8.62656     | 0            | 0            |
+--------------+---------+-------------+--------------+--------------+
...
+--------------+---------+-------------+--------------+--------------+
| 6            | 3316.61 | 8.1067      | -0.519865    | 1.7917595    |
+--------------+---------+-------------+--------------+--------------+
...
0.3952. In our case
$\text{}\frac{x_{1}}{n} = \frac{1}{150} = 0.0067$. Now we need to find the adjusted value as explained in
step-6 in section \@ref(smith) using the table below.The value of
...
+------------+---------------+-------------+
| Computed   | 0.001 to 0.01 | 0.01 to 0.1 |
|            |               |             |
| *b*        |               |             |
+============+===============+=============+
| 1.00       | 1.000         | 1.000       |
...
If $b$ value is low it indicates a relatively high degree of correlation among adjacent plots in the study area indicating a gradual change in soil fertility. But here $b$ is 0.43892, which is moderate, so there is no indication of gradient but slight indication of patches.  

### Maximum Curvature Method
This method (see section \@ref(mcm)) is used to find optimum plot size.  A curve is plotted by taking the plot size (in terms of basic units) on X-axis and the CV values on the Y-axis.  
...
Figure \@ref(fig:mcm1)indicates that as the plot size increases, coefficients of variation decreases and this decrease is maximum with the square shape plot of 5m×5m. As we took a small data set for illustrative purpose, it is clearly seen that 5m×5m plot has the lowest CV value and also lowest variance per basic unit area.  
...
Figure \@ref(fig:mcm2) shows the relationship between variance per basic unit area *V*~x~ and plot size (*x*)  
 \
 \
 

::: {#hello .greeting .message style="color: #c9c6c5;"}
<center>**"The business of the statistician is to catalyze the scientific learning process." :– George Box.**</center>
:::
